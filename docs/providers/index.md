---
summary: "Model providers (LLMs) supported by Velaclaw"
read_when:
...
Velaclaw can use many LLM providers. Pick a provider, authenticate, then set the
...
1. Authenticate with the provider (usually via `velaclaw onboard`).
